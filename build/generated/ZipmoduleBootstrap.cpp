...
#define TAG "com.logic8.zipmodule"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Zipmodule_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Zipmodule.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::ZipmoduleBindings::lookupGeneratedInit(
...
static void Zipmodule_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Zipmodule_getBinding)->GetFunction());
}

static void Zipmodule_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::ZipmoduleBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry ZipmoduleBinding = {
	"com.logic8.zipmodule",
	Zipmodule_init,
	Zipmodule_dispose
};
...
Java_com_logic8_zipmodule_ZipmoduleBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.logic8.zipmodule", &ZipmoduleBinding);
	titanium::KrollBindings::addExternalLookup(&(::ZipmoduleBindings::lookupGeneratedInit));
}
